...
class ConfirmMomoPaymentAction
{
    ...
    public function execute(Payment $payment, string $transactionId, int $amount): Payment
    {
        $confirmedPayment = DB::transaction(function () use ($payment, $transactionId, $amount): Payment {
            ...
            if ($lockedPayment->provider !== 'momo') {
            ...
            if ($amount !== (int) $lockedPayment->amount) {
                throw ValidationException::withMessages(['payment' => 'Số tiền MoMo không khớp với đơn hàng Clare.']);
            }
            ...
            PaymentStatusHistory::query()->create([
                'payment_id' => $lockedPayment->getKey(), 'from_status' => $previousStatus, 'to_status' => 'paid', 'changed_by' => null,
                'note' => 'MoMo IPN có chữ ký hợp lệ đã xác nhận thanh toán thành công.',
            ]);
